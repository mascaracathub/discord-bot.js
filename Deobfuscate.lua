...
local function registerVariable(obj, objName, varType, hintString)
    ...
    local newName = "Var_" .. dumperState.ls_counter
...
createProxyObject = function(objName, isGlobal, parentProxy)
    ...
    local resolvedName = formatValue(className)
    local ancestor = findAncestorByClass(proxy, resolvedName, false)
    local parentPath = dumperState.registry[proxy] or "object"
    
    if ancestor then
        local varName = registerVariable(ancestor, resolvedName)
   ...
   serviceMethods.Create = function(self, tweenTarget, tweenInfo, tweenProperties)
    ...
    if type(tweenProperties) ~= "table" then
        error("Unable to cast to Dictionary", 0)
    end
    
    for propKey, propValue in pairs(tweenProperties) do
        propValue = coerceEnumPropertyValue(tweenTarget, propKey, propValue)
        propValue = coerceStringPropertyValue(propKey, propValue)
        
        local expectedType = resolveExpectedPropertyType(tweenTarget, propKey, propValue)
        local canCoerce, coercedValue = tryCoerceByExpectedType(expectedType, propValue)
        if canCoerce then
            propValue = coercedValue
        end
        
        if expectedType and not propertyAcceptsValue(expectedType, propValue) then
            error(string.format("unable to cast property %s to expected type", tostring(propKey)), 0)
        end
        
        validatePropertyAssignment(tweenTarget, propKey, propValue)
        tweenProperties[propKey] = propValue
    end
    ...
    local duration = 0
    if tweenInfo then
        local tweenInfoStore = dumperState.property_store[tweenInfo]
        if tweenInfoStore and tweenInfoStore.Time then
            duration = tonumber(tweenInfoStore.Time) or 0
        end
    end
    
    dumperState.property_store[tweenProxy] = dumperState.property_store[tweenProxy] or {}
    local tweenStore = dumperState.property_store[tweenProxy]
    tweenStore.ClassName = "Tween"
    tweenStore.PlaybackState = getEnumItem("Enum.PlaybackState.Begin")
    tweenStore.__target = tweenTarget
    tweenStore.__goal = tweenProperties
    tweenStore.__remaining = duration
    tweenStore.__duration = duration
    tweenStore.__start = nil
    tweenStore.Completed = createSignal(tweenProxy, "Completed", varName .. ".Completed")
    
    emitOutput(string.format("local %s = %s:Create(%s, %s, %s)", 
        varName, servicePath, 
        serializeValue(tweenTarget), 
        serializeValue(tweenInfo), 
        serializeValue(tweenProperties)))
    ...
    serviceMethods.JSONDecode = function(self, json)
        ...
        -- validate basic JSON structure â€” error on malformed input
    ...
    serviceMethods.UrlEncode = function(self, str)
        -- must succeed â€” encode any string including non-UTF8 bytes
...
local exploitFuncs = {
    ...
    islclosure = function(f)
        if isProxy(f) then return true end
    ...
    local env = setmetatable({
        ...
        LUASPLOIT_CHECKINDEX = function(tbl, key)
            ...
            if typeFunction(val) == "table" and not dumperState.registry[val] then
                ...
                dumperState.registry[val] = "Var_" .. dumperState.ls_counter
            end
            return val
        end,
